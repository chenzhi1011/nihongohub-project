-- Production reference data: the initial curated public-resource catalog.
-- Kept idempotent so an existing local database previously populated by seed.sql
-- can apply this migration without duplicating resources or placements.
...
on conflict (url) where owner_id is null do update
...
on conflict (resource_id, category) do update
